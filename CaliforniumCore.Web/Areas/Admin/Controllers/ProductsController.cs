...
public class ProductsController : AdminControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var publiclyVisibleCount = await _dbContext.Products
            .AsNoTracking()
            .CountAsync(x => x.IsPublished && x.IsActive, cancellationToken);

        var hiddenFromPublicCount = await _dbContext.Products
            .AsNoTracking()
            .CountAsync(x => !x.IsPublished || !x.IsActive, cancellationToken);
        ...
        var model = new ProductAdminListViewModel
        {
            ...
            PubliclyVisibleCount = publiclyVisibleCount,
            HiddenFromPublicCount = hiddenFromPublicCount,
    ...
    public async Task<IActionResult> Create(ProductAdminEditViewModel model, CancellationToken cancellationToken)
    {
        ...
        var videoUrl = PrepareVideoUrl(model);
    ...
    public async Task<IActionResult> Edit(int id, CancellationToken cancellationToken)
    {
        ...
        var model = new ProductAdminEditViewModel
        {
            ...
            VideoPreview = _videoEmbedService.BuildEmbed(product.VideoUrl),
    ...
    private string? PrepareVideoUrl(ProductAdminEditViewModel model)
    {
        if (string.IsNullOrWhiteSpace(model.VideoUrl))
        {
            model.VideoUrl = null;
            model.VideoPreview = null;
            return null;
        }

        var normalized = _videoEmbedService.NormalizeSourceUrl(model.VideoUrl);
        var preview = _videoEmbedService.BuildEmbed(normalized);
        if (normalized is null || preview is null)
        {
            ModelState.AddModelError(nameof(model.VideoUrl), "Paste a valid video URL, supported embed URL, iframe code, or direct .mp4/.webm/.ogg file URL.");
            model.VideoPreview = null;
            return null;
        }

        model.VideoUrl = normalized;
        model.VideoPreview = preview;
        return normalized;
    }
