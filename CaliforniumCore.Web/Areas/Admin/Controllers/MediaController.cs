...
public class MediaController : AdminControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var group = string.IsNullOrWhiteSpace(mediaGroup) ? "Editor" : mediaGroup.Trim();
        var uploadedByUserId = User.Identity?.Name;
        var title = Path.GetFileNameWithoutExtension(file.FileName);

        try
        {
            var media = await _fileUploadService.SaveMediaAsync(
                file,
                group,
                title: title,
                altText: string.IsNullOrWhiteSpace(altText) ? title : altText.Trim(),
                uploadedByUserId: uploadedByUserId,
                cancellationToken: cancellationToken);
