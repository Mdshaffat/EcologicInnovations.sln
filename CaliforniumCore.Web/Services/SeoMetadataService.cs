...
public class SeoMetadataService : ISeoMetadataService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var finalTitle = string.IsNullOrWhiteSpace(title)
            ? siteSetting?.MetaTitleDefault ?? siteSetting?.CompanyName ?? "Californium Core"
            : title;

        var finalDescription = string.IsNullOrWhiteSpace(description)
            ? siteSetting?.MetaDescriptionDefault ?? siteSetting?.Tagline ?? "Californium Core"
            : description;
        ...
        CancellationToken cancellationToken = default)
    {
        if (page is null)
        {
            return await BuildDefaultAsync(
                title: "Page Not Found | Californium Core",
